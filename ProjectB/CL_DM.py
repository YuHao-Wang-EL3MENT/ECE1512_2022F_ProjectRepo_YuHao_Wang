...
def main():
    ...
    parser.add_argument('--method', type=str, default='default', help='random/herding/DSA/DM') #change to (default) to use synthetic dataset
    ...
    parser.add_argument('--syn_data_path', type=str, default='/content/drive/MyDrive/ECE1512/ProjectB/All_Code/result/1000iterMNIST/res_DC_MNIST_LeNet_10ipc.pt', help='synthetic dataset path')
    ...
    for seed_cl in range(1):
        ...
        if args.method == 'random':
            ...
            data = torch.load(args.syn_data_path, map_location='cpu')['data'] #change to synthetic dataset dataset         
            images_train_all = [data[0][0]]
            labels_train_all = [data[0][1]]
            print('use data: MNIST')
        ...
        for step in range(args.steps):
            ...
            for ep_eval in range(args.num_eval):
                ...
                trained_model, acc_train, acc_test = evaluate_synset(ep_eval, net_eval, img_syn_eval, lab_syn_eval, testloader, args)
            ...
            print('Model continute learn', classes_seen, 'classes and evaluate on 3 random network. The average test accuracy is', np.mean(accs))

    print('\n\n')
    print('%d step learning %s done!!!!!!!!!!!!!!!!!!!!!!'%(args.steps, args.method))
